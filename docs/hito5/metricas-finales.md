...
Fecha de medición: 06/06/2026. Estado del repositorio: rama main, CI en verde.
...
Todos los métodos cumplen CC ≤ 10, el umbral del Plan SQA.
...
MI ≈ 85, que en la escala SEI corresponde a alta mantenibilidad (≥ 85 alta, 65–85 moderada, < 65 baja).

El Volumen de Halstead se estima, ya que la toolchain actual (PMD y JaCoCo) no lo calcula. El valor es consistente con los indicadores medibles: CC ≤ 10, alrededor de 29 LOC por clase y cobertura del 100 % en los módulos principales. Para un valor exacto se requeriría SonarQube o Radon.
...
| model/ | 100 % |
| dto/ | 91,7 % |
| config/ (incluye GlobalExceptionHandler) | 25,8 % |
| security/ | 2,2 % |
| TOTAL | 53,0 % |
...
| Defectos corregidos | 9 (incluido el crítico de RBAC) |
...
Una densidad alta en este caso refleja un testing exhaustivo (se buscaron defectos de forma activa, incluido el testing exploratorio), no una baja calidad: la mayoría son menores y están documentados con su severidad y estado.

## 6. Evolución Hito 3 → Hito 4 → Hito 5
...
| Defectos corregidos | 0 | 5 | 9 |
| Pipeline CI | no | sí | sí |
